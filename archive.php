...
if (is_month()) {
  ...
  $archive_title = sprintf('%d年%d月', $archive_year, $archive_month);
  $section_title = 'Archive';
  $section_subtitle = 'アーカイブ';
} elseif (is_year()) {
  // Yearly archive
  $archive_year = get_query_var('year');
  $archive_title = sprintf('%d年', $archive_year);
  $section_title = 'Archive';
  $section_subtitle = 'アーカイブ';
} else {
  // Other archive types
  ...
  <div class="c-archive-wrapper">
    <div class="a-archive-main">
      <div class="c-archive-titlewrap">
        <h2 class="c-archive-title"><?php echo esc_html($archive_title); ?></h2>
      </div>
